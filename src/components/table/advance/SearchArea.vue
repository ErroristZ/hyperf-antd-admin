                ...
                <div class="search-item" :key="index" v-for="(col, index) in searchCols">
                        <div v-if="col.dataType === 'boolean'"
                             :class="['title', {active: col.search.value !== undefined}]">
                                ...
                                <a-switch @change="onSwitchChange(col)" class="switch" v-model="col.search.value"
                                          size="small"
                                ...
                                />
                                <a-icon v-if="col.search.value !== undefined" class="close"
                                        @click="e => onCloseClick(e, col)" type="close-circle" theme="filled"/>
                        </div>
                        <div v-else-if="col.dataType === 'time'" :class="['title', {active: col.search.value}]">
                                ...
                                <a-time-picker :format="col.search.format" v-model="col.search.value" placeholder="选择时间"
                                               ...
                                               @openChange="open => onCalendarOpenChange(open, col)" class="time-picker"
                                               size="small" :get-popup-container="() => $refs.root"/>
                        </div>
                        <div v-else-if="col.dataType === 'date'" :class="['title', {active: col.search.value}]">
                                ...
                                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                                <a-date-picker :format="col.search.format" v-model="col.search.value"
                                               @change="onDateChange(col)" class="date-picker" size="small"
                                               :getCalendarContainer="() => $refs.root"/>
                        </div>

                        <div v-else-if="col.dataType === 'dateRange'" :class="['title', {active: col.search.value}]">
                                <template v-if="col.title">
                                        {{col.title}}:
                                </template>
                                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                                <a-range-picker :format="col.search.format"
                                                v-model="col.search.value"
                                                :open="col.search.visible"
                                                @change="(date, dateStr) => onCalendarChange(date, dateStr, col)"
                                                @openChange="open => onCalendarOpenChange(open, col, 1)"
                                                class="date-range" size="small"
                                                :getCalendarContainer="() => $refs.root"
                                >
                                        <template slot="renderExtraFooter">
                                                <a-button size="small" @click="onCalendarOpenChange(false, col, 2)">关闭
                                                </a-button>
                                        </template>
                                </a-range-picker>
                        </div>

                        <div v-else-if="col.dataType === 'datetimeRange'"
                             :class="['title', {active: col.search.value}]">
                                ...
                                <a-range-picker :format="col.search.format"
                                                ...
                                                @change="(date, dateStr) => onCalendarChange(date, dateStr, col)"
                                                @openChange="open => onCalendarOpenChange(open, col, 1)"
                                                class="datetime-range" size="small" show-time
                                                :getCalendarContainer="() => $refs.root"
                                                @ok="rangePickerOk(col)"
                                >
                                        <template slot="renderExtraFooter">
                                                <a-button size="small" @click="onCalendarOpenChange(false, col, 2)">关闭
                                                </a-button>
                        ...
                        <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
                                ...
                                <a-date-picker :format="col.search.format" v-model="col.search.value"
                                               ...
                                               class="datetime-picker" size="small" show-time
                                               :getCalendarContainer="() => $refs.root"/>
                        </div>
                        <div v-else-if="col.dataType === 'select'"
                             :class="['title', {active: col.search.value !== undefined}]">
                                <template v-if="col.title">
                                        {{col.title}}:
                                </template>
                                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                                <a-select :allowClear="true" :options="col.search.selectOptions"
                                          :mode="col.search.mode ? col.search.mode : 'default'"
                                          v-model="col.search.value" placeholder="请选择..." @change="onSelectChange(col)"
                                          class="select" slot="content" size="small"
                                          :get-popup-container="() => $refs.selectRoot">
                                </a-select>
                        </div>
                        <div v-else :class="['title', {active: col.search.value}]">
                                <a-popover @visibleChange="onVisibleChange(col, index)" v-model="col.search.visible"
                                        ...
                                        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                                        <div class="value " v-if="col.search.value">:&nbsp;&nbsp;{{col.search.format &&
                                                typeof col.search.format === 'function' ?
                                                col.search.format(col.search.value) : col.search.value}}
                                        </div>
                                        ...
                                        <div class="operations" slot="content">
                                                ...
                                                <a-button @click="onConfirm(col)" class="btn" size="small"
                                                          type="primary">确认
                                                </a-button>
                                        </div>
                                        <div class="search-overlay" slot="title">
                                                <a-input :id="`${searchIdPrefix}${index}`" :allow-clear="true"
                                                         @keyup.esc="onCancel(col)" @keyup.enter="onConfirm(col)"
                                                         v-model="col.search.value" size="small"/>
                                        </div>
                                </a-popover>
                                <a-icon v-if="col.search.value" @click="e => onCloseClick(e, col)" class="close"
...
<script>
        ...
        export default {
                ...
                methods: {
                        ...
                        rangePickerOk(col) {
                                col.search.rangePickerOk = true;
                        },
                        onCalendarOpenChange(open, col, type) {
                                if (type === 1 && !col.search.rangePickerOk) {
                                        open = true;
                                } else if (type === 2 && col.dataType === 'datetimeRange') {
                                        if (col.search.backup && col.search.backup._i.length > 0) {
                                                col.search.value = col.search.backup._i;
                                        } else {
                                                col.search.value = undefined;
                                        }
                                }
                                col.search.rangePickerOk = false;
                                col.search.visible = open
                                if (this.$parent && this.$parent.$parent && this.$parent.$parent.$parent && this.$parent.$parent.$parent.$refs.table) {
                                        if (open) {
                                                this.$parent.$parent.$parent.$refs.table.style.overflow = 'auto';
                                        } else {
                                                this.$parent.$parent.$parent.$refs.table.style.overflow = '';
                                        }
                                }
                                if (col.dataType === 'dateRange') {
                                        return false;
                                }
                                ...
                                columns.filter(item => item.search.value !== undefined && item.search.value !== '' && item.search.value !== null)
                                        .forEach(col => {
                                                ...
                                                if (this.formatConditions && format) {
                                                        ...
                                                        } else if (typeof format === 'string' && value.constructor.name === 'Array') {
                                                                const length = value.length;
                                                                conditions[col.dataIndex] = [];
                                                                for (var i = 0; i < length; ++i) {
                                                                        conditions[col.dataIndex].push(value[i].format(format));
                                                                }
                                                        } else {
                        ...
                        momentEqual(target, source, format) {
                                if (target === source) {
                                        return true
                                } else if (target && target.length === undefined && source && target.format(format) === source.format(format)) {
                                        return true
                                } else if (target && target.length && source && source._i && source._i.length) {
                                        const length = target.length;
                                        for (var i = 0; i < length; ++i) {
                                                if (target[i].format(format) !== source._i[i].format(format)) {
                                                        return false;
                                                }
                                        }
                                        return true;
