          ...
          <a-col :xl="{span: 10, offset: 2}" :lg="{span: 12}" :md="{span: 12}" :sm="24">
            <a-form-item label="状态">
              <a-switch
                ...
                :checked="is_display === 0 ? true : false"
                @change="handleSwitch"
              />
            </a-form-item>
          </a-col>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      const that = this
      request(MENU + '/menu?id=' + that.id, METHOD.GET, {})
        .then(function (result) {
          ...
          that.form.setFieldsValue({
            parent_id: result.data.data.parent_id ? result.data.data.parent_id : undefined,
          ...
          that.is_display = result.data.data.is_display
    ...
    handleSend() {
      ...
      sendData.is_display = that.is_display
      sendData.id = that.id
      sendData.menuType = that.menuType
      sendData.method = that.method
      request(MENU + '/edit', METHOD.PUT, sendData)
        .then(function (result) {
          if (result.data.code !== 200) {
            that.confirmLoading = false
            that.$message.error(result.data.message, 3)
    ...
    resetForm() {
      const that = this
      that.parent_id = undefined //父id
      that.is_display = 1 //状态
