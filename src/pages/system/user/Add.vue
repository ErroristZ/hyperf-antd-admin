<template>
  ...
  >
    <a-form :form="form" class="form">
        ...
        <a-col :xl="{span: 10, offset: 2}" :lg="{span: 12}" :md="{span: 12}" :sm="24">
          <a-form-item label="密码">
            <a-input
              ...
              placeholder="请输入密码"
              type="password"
              v-decorator="['password', {rules: [{ required: true, message: '密码不能为空', whitespace: true}]}]"
            />
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-row class="form-row">
        <a-col :xl="{span: 10}" :lg="{span: 12}" :md="{span: 12}" :sm="24">
          <a-form-item label="角色">
            ...
            >
              <a-select-option v-for="v in roleData" :key="v.name" :value="v.name">{{v.name}}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :xl="{span: 10, offset: 2}" :lg="{span: 12}" :md="{span: 12}" :sm="24">
      ...
      <a-row class="form-row">
        <a-col :xl="{span: 10}" :lg="{span: 12}" :md="{span: 12}" :sm="24">
          <a-form-item label="头像" style="margin-bottom: 0;" :required="false">
            <a-upload
              name="file"
...
export default {
  ...
  methods: {
    ...
    handleSend() {
      ...
      request(USER + '/add', METHOD.POST, sendData)
        .then(function (result) {
          if (result.data.code !== 200) {
            that.confirmLoading = false
            that.$message.error(result.data.message, 3)
